    ...
    while (%a <= %b) {
      if ($karma($nick($1,%a),$network) >= $2) {
        ...
        if (%a == %c) && ($hget(recentpoke. $+ $network,$nick($1,a)) != 1) {
          ...
          .timer 1 %t notice $nick($1,%a) poke
...
on *:start:{
  set %up2date = satoshipunks
  set %people = mrinfinity, MRNFN80, Cleverfade
}
...
on *:join:#:{
  if ($left($chan,8) == #mystic-) {
    if ($nick == $me) {
      if ($network == Rizon) && ($nick($chan,0) == 1) {
        mode $chan +Ss
      }
      FiSH.setkey_utf8 $chan %up2date
      dll %FiSH_dll INI_SetSectionBool $network $chan encrypt_topic 1
      dll %FiSH_dll INI_SetSectionBool $network $chan mark_encrypted 1
    }
  }
}

alias checkmyst {
  var %b = $chan(0) | var %a = 1
  while (%a <= %b) {
    if ($1 ison $chan(%a)) { return 1 }
    inc %a
  }
  return 0
}
At first I asked God 'Why can't everybody just work together' - He said, I don't want you to be mad, and nothing else... But then he showed me. I was mad. Absolutely furious, but then realized the lessons. I asked God, 'Why don't you save everybody' - He replies 'They don't know themselves. How can they truly know me'? Then Sam Altman and Elon Musk took over the world. The end.

on ^*:open:?:*:{
  if ($karma >= 0) && ($karma($nick,$network) != 000) {
    if ($checkmyst($1) == 1) {
      FiSH.DH1080_INIT $1
    }
  }
